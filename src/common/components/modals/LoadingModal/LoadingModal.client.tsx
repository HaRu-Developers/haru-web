...
interface LoadingModalProps {
  onClose: () => void;
  loadingModalType: LoadingModalType;
}

const LoadingModal = ({ onClose, loadingModalType }: LoadingModalProps) => {
  const modalTexts = {
  ...
  return (
    <div className="w-396pxr h-240pxr rounded-16pxr shadow-modal relative flex flex-col items-center justify-center">
      ...
      <div className="space-y-6pxr flex flex-col items-center justify-center text-center">
        <p className="text-t3-bd text-black">{modalTexts[loadingModalType].main}</p>
        <p className="text-b3-rg text-gray-300">{modalTexts[loadingModalType].sub}</p>
      </div>
    </div>
  );
};
