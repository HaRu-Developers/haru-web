...
export const TimePicker = ({ onTimeSelect, timeToDisableBefore }: TimePickerProps) => {
  ...
  const isTimeDisabled = (time: string): boolean => {
    if (!timeToDisableBefore) return false;
    const [hh, mm] = time.split(':');
    const timeDate = new Date(timeToDisableBefore);
    timeDate.setHours(parseInt(hh, 10), parseInt(mm, 10));
    return timeDate < timeToDisableBefore;
  };
              ...
              className={clsx(
                'text-b2-rg py-5pxr flex w-55 items-center justify-center rounded-lg border transition-colors duration-150',
                {
                  ...
                  'hover:bg-primary-selected border-transparent bg-gray-700 text-black':
                    selectedTime !== time,
                  'cursor-not-allowed opacity-50': isTimeDisabled(time),
                },
              )}
              role="option"
