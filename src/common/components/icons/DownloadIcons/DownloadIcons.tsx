...
const DownloadIcons = ({ state, className }: IconsCommonProps<DownloadIconsState>) => {
  switch (state) {
    case DownloadIconsState.DOWNLOAD_BLACK:
      return <DownloadIcon className={clsx('h-[24px] w-[24px] text-black', className)} />;
    case DownloadIconsState.DOWNLOAD_WHITE:
      return <DownloadIcon className={clsx('h-[24px] w-[24px] text-white', className)} />;
    case DownloadIconsState.FILE_DOWNLOAD_PDF:
      return <DownloadFileIcon className={clsx('h-[20px] w-[20px] text-[#F83E33]', className)} />;
    case DownloadIconsState.FILE_DOWNLOAD_WORD:
      return <DownloadFileIcon className={clsx('h-[20px] w-[20px] text-[#CD53F3]', className)} />;
    default:
      return null;
  }
};
